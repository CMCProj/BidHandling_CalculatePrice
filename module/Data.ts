...
export class Data {
    constructor(item: string, constructionNum: string, workNum: string, detailWorkNum: string, code: string, name: string, standard: string, unit: string, quantity: number, materialUnit: number, laborUnit: number, expenseUnit: number) {
        this.Item = item
    ...
    /**재료비 단가*/
    public get MaterialUnit(): number {     //사용자가 단가 정수처리를 원한다면("2") 정수 값으로 return / Reset 함수를 쓰지 않은 경우의 조건 추가 (23.02.06)
