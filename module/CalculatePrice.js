...
class CalculatePrice {
    static Calculation() {
        const bidString = fs.readFileSync("C:\\\\Users\\joung\\OneDrive\\문서\\AutoBID" + "\\OutputDataFromBID.json", 'utf-8');
    ...
    static Reset() {
        ...
        let DM = Data_1.Data.Investigation['직접재료비'];
        let DL = Data_1.Data.Investigation['직접노무비'];
        let OE = Data_1.Data.Investigation['산출경비'];
    ...
    static GetFixedPriceRate() {
        //고정금액 비율 계산
        const directConstPrice = Data_1.Data.Investigation['직공비'];
        const fixCostSum = Data_1.Data.InvestigateFixedPriceDirectMaterial + Data_1.Data.InvestigateFixedPriceDirectLabor + Data_1.Data.InvestigateFixedPriceOutputExpense;
    ...
    static FindMyPercent() {
        if (Data_1.Data.FixedPricePercent < 20.0) //고정금액 < 20%
            this.myPercent = 0.85;
        else if (Data_1.Data.FixedPricePercent < 25.0) //고정금액 < 25%
            this.myPercent = 0.84;
        else if (Data_1.Data.FixedPricePercent < 30.0) //고정금액 < 30%
            this.myPercent = 0.83;
        else
        ...
        for (let key in bidT3) {
            ...
            if (code !== undefined && type === 'S') {
                ...
                if (curObject.Item.localeCompare('일반')) {
                    ...
                    Data_1.Data.RealDirectLabor -= +JSON.stringify(bidT3[key]['C21']['_text']).slice(1, -1);
                    Data_1.Data.RealOutputExpense -= +JSON.stringify(bidT3[key]['C22']['_text']).slice(1, -1);
                    let targetPrice = (curObject.MaterialUnit + curObject.LaborUnit + curObject.ExpenseUnit) * this.targetRate; //Target 단가 합계
                    ...
                    if (Data_1.Data.LaborCostLowBound.localeCompare('1')) { //노무비 하한 80% 적용 O
    ...
    static SetExcludingPrice() {
        ...
        let TempInvestDirectSum = Data_1.Data.Investigation['직공비']; //조사직공비
    ...
    static SubstitutePrice() {
        ...
        for (let key in bidT5) {
            if (bidT5[key]['C4']['_text'].slice(1, -1) !== '이윤' && Data_1.Data.Bidding[bidT5[key]['C4']['_text'].slice(1, -1)]) {
                bidT5[key]['C8']['_text'] = Data_1.Data.Bidding[bidT5[key]['C4']['_text'].slice(1, -1)].toString();
            }
    ...
    static CreateFile() {
        ...
        //생성된 입찰내역 파일 압축 
        const files = fs.readdirSync("C:\\\\Users\\joung\\OneDrive\\문서\\AutoBID");
