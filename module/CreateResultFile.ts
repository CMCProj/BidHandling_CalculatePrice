...
namespace SetUnitPriceByExcel
{
    class CreateResultFile
    {
        public static Create(): void
        {
            ...
            fs.readdir(directoryPath, function(err, files){ // 파일 경로에 있는 엑셀파일의 절대경로를 모두 저장한다.
                if(err){
            ...
            xlsList.forEach((xls) => { // 기존의 엑셀 파일을 모두 삭제한다.
            ...
            Data.Dic.forEach(function(dic){
                const workbook = worksheet.xls.readFile('입찰내역.xls');
