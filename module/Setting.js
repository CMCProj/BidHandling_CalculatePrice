...
class Setting {
    ...
    static AddConstructionList() {
        Data_1.Data.ConstructionNums.forEach((key, value) => Data_1.Data.Dic.set(key, new Array())) // Data.Dic자료구조 체크하기 <string. Data[]>가 맞는지, <string, Array[]>로 해야하는지
    }
    static GetItem(bid) {
        let item = null
    ...
    static MatchConstructionNum(filePath) {
        ...
        const workbook = ExcelHandling_1.ExcelHandling.GetWorkbook(filePath, '.xlsx')
        ...
        Data_1.Data.Dic.forEach((value, key) => {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                let sameName = value[i].Name === row.GetCell(4).StringCallValue
                if (sameName) check++
                if (check == 3) {
    ...
    static CopyFile(filePath) {
        ...
        Data_1.Data.Dic[constNum].forEach((curObj) => {
            ...
            while (true) {
                ...
                let quantity = 0.0
                try {
                    let quantity = Number(row.GetCell(7).NumericCellValue)
                } catch (_a) {
                    rowIndex++
    ...
    static SetUnitPriceNoExcel() {
        ...
        for (let key in bidT3) {
            if (
                ...
                let curObject = Data_1.Data.Dic[constNum].Find(
        ...
        if (fs.existsSync(Data_1.Data.work_path + '\\Setting_Xml.xml')) {
            fs.unlink(Data_1.Data.work_path + '\\Setting_Xml.xml', (err) => {
                if (err.code == 'ENOENT') {
        ...
        fs.writeFileSync(
            Data_1.Data.work_path + '\\Setting_Xml.xml',
    ...
    static GetRate() {
        ...
        for (let key in bidT5) {
            ...
            let val1 = JSON.stringify(bidT5[key]['C6']['_text'])
            let val2 = JSON.stringify(bidT5[key]['C7']['_text'])
            if (JSON.stringify(bidT5[key]['C5']['_text']) === '7') {
                let fixedPrice = Number(JSON.stringify(bidT5[key]['C8']['_text']))
