...
export class CalculatePrice {
    ...
    public static SubstitutePrice(): void { //BID 파일 내 원가계산서 관련 금액 세팅
        ...
        for (let key in bidT5) {
            if (bidT5[key]['C4']['_text'] !== '이윤' && Data.Bidding[bidT5[key]['C4']['_text']]) {
                bidT5[key]['C8']['_text'] = Data.Bidding[bidT5[key]['C4']['_text']].toString();
            }
    ...
    public static CreateZipFile(xlsfiles: Array<string>): void {
        if (fs.existsSync(Data.folder + '\\WORK DIRCETORY\\입찰내역.zip')) {//기존 입찰내역.zip 파일은 삭제한다. (23.02.02)
